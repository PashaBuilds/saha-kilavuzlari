...
## MicroBlaze: MMI üretme ve yazılımı sentezsiz gömme

```tcl
# MMI, YERLESTIRILMIS BRAM konumlarini yazar -> implementasyon ister:
open_run impl_1
write_mem_info -force sistem.mmi
```

```bash
# ELF'i bitstream'e göm (sentez YOK — Bölüm 9):
updatemem -meminfo sistem.mmi -data app.elf \
          -bit sistem_wrapper.bit -proc sistem_i/microblaze_0 \
          -out download.bit
```

Repo'da hazır: `vivado/microblaze/impl_bitstream.tcl` implementasyon +
bitstream + MMI + kaynak kullanım raporunu tek koşuda üretir.
